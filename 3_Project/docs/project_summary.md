...
- **Trevisan-based key derivation** produced **disjoint AES-256 session keys** from the compressed master key, ensuring **no reuse of key material**, while **HKDF-SHA256** expanded the same master key into multiple cryptographically independent AES-256 session keys for strong per-session confidentiality.
